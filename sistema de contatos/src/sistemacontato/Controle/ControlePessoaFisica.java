        ...
        try {
            try {
                ...
                while(input.hasNext()) {
                    ...
                    pessoaFisica.setCategoria(input.next());
                    arrayListPessoaFisica.add(pessoaFisica);
                }
            } finally {
                if(input != null) {
        ...
        try {
            try {
                ...
                for (int i = 0; i < arrayListPessoaFisica.size(); i++) {
                    ...
                    output.format("%s %s %s %s %s %s %s %s\n", pessoaFisica.getNome(), pessoaFisica.getEmail(), pessoaFisica.getTelefoneResidencial(), pessoaFisica.getTelefoneTrabalho(), pessoaFisica.getTelefoneCelular(), pessoaFisica.getFAX(), pessoaFisica.getCPF(), pessoaFisica.getCategoria());
                }
            } finally {
                if (output != null) {
                    output.close();
