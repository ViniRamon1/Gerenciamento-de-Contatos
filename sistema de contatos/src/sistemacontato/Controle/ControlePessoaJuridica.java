        ...
        try {
            try {
                ...
                while(input.hasNext()) {
                    ...
                    pessoaJuridica.setCategoria(input.next());
                    arrayListPessoaJuridica.add(pessoaJuridica);
                }
            } finally {
                if(input != null) {
        ...
        try {
            try {
                ...
                for (int i = 0; i < arrayListPessoaJuridica.size(); i++) {
                    ...
                    output.format("%s %s %s %s %s %s %s %s\n", pessoaJuridica.getNome(), pessoaJuridica.getEmail(), pessoaJuridica.getTelefoneResidencial(), pessoaJuridica.getTelefoneTrabalho(), pessoaJuridica.getTelefoneCelular(), pessoaJuridica.getFAX(), pessoaJuridica.getCNPJ(), pessoaJuridica.getCategoria());
                }
            } finally {
                if (output != null) {
                    output.close();
