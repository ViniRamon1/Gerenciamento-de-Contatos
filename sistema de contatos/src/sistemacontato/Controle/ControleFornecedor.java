        ...
        try {
            try {
                ...
                while(input.hasNext()) {
                    ...
                    fornecedor.setCategoria(input.next());
                    arrayListFornecedor.add(fornecedor);
                }
            } finally {
                if(input != null) {
        ...
        try {
            try {
                ...
                for (int i = 0; i < arrayListFornecedor.size(); i++) {
                    ...
                    output.format("%s %s %s %s %s %s\n", fornecedor.getNome(), fornecedor.getEmail(), fornecedor.getCNPJ(), fornecedor.getTelefoneFixo(), fornecedor.getFAX(), fornecedor.getCategoria());
                }
            } finally {
                if (output != null) {
                    output.close();
